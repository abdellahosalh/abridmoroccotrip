...
BACKUP_FOLDER = os.path.join(FOLDER, "backup_force_update")

# New Header (exactly as in your new index.html)
...
def update_file(filepath):
    ...
    # Replace <head> fonts and style.css
    head_start = content.find('<head>')
    head_end = content.find('</head>')
    if head_start != -1 and head_end != -1:
        head_content = content[head_start:head_end+7]
        # Always ensure fonts and style.css are present
        if 'Syne' not in head_content:
            # Remove any existing font links
            head_content = re.sub(r'<link[^>]*fonts\.googleapis\.com[^>]*>', '', head_content)
            head_content = re.sub(r'<link[^>]*fonts\.gstatic\.com[^>]*>', '', head_content)
  ...
  <link href="https://fonts.googleapis.com/css2?family=Syne:wght@400;500;600;700;800&family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">'''
        ...
        if 'style.css' not in head_content:
            head_content = head_content.replace('</head>', '  <link rel="stylesheet" href="style.css">\n</head>')
        content = content.replace(content[head_start:head_end+7], head_content)
...
print("🚀 FORCE UPDATING ALL HTML FILES...")
updated = 0
...
print(f"\n✅ Updated {updated} files. Backups in {BACKUP_FOLDER}")
print("🌟 Now upload all HTML files to your server and clear cache.")

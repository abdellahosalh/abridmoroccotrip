...
SKIP_FILES = [
    ...
    "trip-template.html",
    "agent-tools.html",
    "full_update.py",  # itself
]

# ============================================================
# TEMPLATE: NEW HEADER (exactly as in index.html)
# ============================================================
...
# ============================================================
# TEMPLATE: NEW FOOTER (exactly as in index.html)
# ============================================================
...
def update_head(content):
    """Replace the <head> section with the new one (fonts, style.css, meta)"""
    # Remove old font links and add new ones
    # We'll just ensure the correct font and style.css are referenced.
    # We'll inject the new head content if missing.
    if 'Syne' not in content:
        # Find the head tag and inject fonts
        ...
        if head_start != -1 and head_end != -1:
            ...
            if 'style.css' not in head_content:
                # Add style.css link after fonts
...
def replace_header_footer(content):
    """Replace the existing header and footer with the new ones"""
    # Remove old header: match from <header class="site-header"> to </header>
    header_pattern = re.compile(r'<header\s+class="site-header"[^>]*>.*?</header>', re.DOTALL)
    content = header_pattern.sub(NEW_HEADER, content)

    # Remove old footer: match from <footer to </footer> at the end
    footer_pattern = re.compile(r'<footer[^>]*>.*?</footer>', re.DOTALL)
    # But we want to keep the </body> after, so we'll replace the last footer.
    # We'll find all footers and replace the last one, but safer: replace everything from the start of footer to the end of the file.
    # We'll locate the first '<footer' and then find the matching '</footer>' and replace that block.
    # Easier: find the last occurrence of '</footer>' and replace the part before it.
    # Actually, we'll just remove all footers and append our new one just before </body>
    # But we need to preserve any scripts after footer.
    # Let's do a safer approach: remove everything from the first '<footer' that is not inside a script tag? Too complex.
    # We'll assume the footer is the last tag before </body>
    # So we find the last '</footer>' and replace from the preceding '<footer' to that '</footer>' with our new footer.
...
def remove_prices(content):
    """Replace currency amounts with 'Contact for pricing'"""
